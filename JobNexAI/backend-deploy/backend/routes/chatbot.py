...
def check_ai_limit(user_id, usage_type='general', daily_limit=10):
    user = get_user_by_id(user_id)
    if user and user.get('role') == 'admin':
        return None
    usage = get_daily_usage(user_id, usage_type)
...
def chat():
    try:
        ...
        limit_err = check_ai_limit(user_id, usage_type='chat', daily_limit=10)
        if limit_err:
        ...
        increment_daily_usage(user_id, usage_type='chat')
...
def cv_analysis():
    try:
        ...
        limit_err = check_ai_limit(user_id, usage_type='chatbot_other', daily_limit=5)
        if limit_err:
            return jsonify({'error': limit_err, 'limit_exceeded': True}), 429
...
Be specific and actionable. Provide before/after rewrites for weak bullet points."""
        ...
        increment_daily_usage(user_id, usage_type='chatbot_other')
