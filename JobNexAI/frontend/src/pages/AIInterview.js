...
const TypingIndicator = () => (
  <div className="flex justify-start mb-4">
    <div className="flex items-start gap-2.5 max-w-[85%]">
      ...
      <div className="glass rounded-2xl rounded-tl-md px-4 py-3 shadow-sm">
...
const AIInterview = () => {
  ...
  if (step === 'setup') {
    return (
      <div className="min-h-screen py-6 sm:py-8 rounded-[2rem] mx-2 sm:mx-4">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-8 text-center">
            <div className="w-16 h-16 bg-gradient-to-br from-accent-cyan via-primary-500 to-accent-violet rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg">
          ...
          <div className="glass bg-amber-500/10 border-amber-500/20 rounded-2xl p-4 mb-6 flex items-start gap-3 shadow-sm">
          ...
          <div className="bento-card-glass p-4 sm:p-8">
                ...
                <div className="glass rounded-2xl p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {user.skills.split(',').map((s, i) => (
                      <span key={i} className="px-2.5 py-1 bg-white/70 border border-white/20 rounded-lg text-sm text-gray-700">{s.trim()}</span>
                    ))}
                  </div>
                </div>
              )}
              {error && <p className="text-red-500 text-sm">{error}</p>}
              <button onClick={handleStart} disabled={loading || !jobTitle.trim()} className="btn-glow w-full py-3 flex items-center justify-center gap-2">
  ...
  return (
    <div className="min-h-screen flex flex-col rounded-[2rem] mx-2 sm:mx-4">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 max-w-3xl mx-auto w-full">
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} mb-4 last:mb-0`}>
            <div className={`flex ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'} items-start gap-2.5 max-w-[85%]`}>
              ...
              <div>
                <div className={`px-4 py-2.5 shadow-sm ${msg.role === 'user' ? 'bg-gradient-to-r from-primary-500 to-accent-violet text-white rounded-2xl rounded-tr-md' : 'glass rounded-2xl rounded-tl-md'}`}>
        ...
        {error && step === 'interview' && (
          <div className="bg-red-50/70 backdrop-blur-xl border border-red-200/50 rounded-xl p-3 mb-4">
      ...
      {step === 'interview' && !result && (
        <div className="glass-strong rounded-b-2xl border-t border-white/20 px-4 py-4">
      ...
      {step === 'results' && (
        <div className="glass-strong rounded-b-2xl border-t border-white/20 px-4 py-4">
          <div className="max-w-3xl mx-auto flex flex-col sm:flex-row gap-2 sm:gap-3">
            ...
            <button onClick={() => window.location.href = '/dashboard'} className="flex-1 px-6 py-3 btn-outline text-center">
              Back to Dashboard
            </button>
          </div>
        </div>
