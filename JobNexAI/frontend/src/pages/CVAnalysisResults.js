...
const CVAnalysisResults = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] pb-16 rounded-[2rem] mx-2 sm:mx-4">
      {/* Premium Header */}
      <div className="bg-gradient-to-br from-gray-950 via-[#0a0a1a] to-[#120a2a] pt-6 sm:pt-8 pb-10 sm:pb-12 relative overflow-hidden border-b border-white/10">
        <div className="ambient-blob w-72 h-72 bg-accent-cyan/10 top-[-20%] right-[-10%]" />
        <div className="ambient-blob w-72 h-72 bg-accent-violet/10 bottom-[-30%] left-[-10%]" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
            <button
              ...
              className="flex items-center text-gray-400 hover:text-white transition-colors font-medium text-xs sm:text-sm group"
            >
              <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white/5 flex items-center justify-center mr-2 sm:mr-3 group-hover:bg-white/10 transition-colors">
            ...
            <div className="flex flex-wrap gap-2 sm:gap-3 w-full sm:w-auto">
              <button onClick={() => navigate('/cv-upload')} className="px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl glass text-gray-200 font-medium hover:bg-white/10 transition-colors text-xs sm:text-sm flex-1 sm:flex-none text-center">
                Analyze Another
              </button>
              <button onClick={() => navigate('/recommendations')} className="px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl bg-gradient-to-r from-accent-cyan to-primary-500 text-white font-medium hover:shadow-lg hover:shadow-accent-cyan/20 transition-all text-xs sm:text-sm flex-1 sm:flex-none text-center">
                View Recommendations
              </button>
            </div>
          </div>

          <div className="flex items-center gap-4 sm:gap-5 mt-6 sm:mt-8">
            <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br from-accent-cyan via-primary-500 to-accent-violet rounded-xl sm:rounded-2xl flex items-center justify-center shadow-lg shadow-accent-cyan/20 text-white transform -rotate-3 flex-shrink-0">
              <svg className="w-6 h-6 sm:w-8 sm:h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
              </svg>
            </div>
            <div>
              <h1 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight gradient-text-strong">AI Resume Analysis</h1>
              <p className="text-gray-400 mt-0.5 sm:mt-1 text-sm sm:text-lg">We've broken down your resume into actionable insights</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6">
        ...
        {resultData.ats_score?.breakdown && (
          <div className="bg-white rounded-[2rem] mx-2 sm:mx-4 shadow-sm border border-gray-100 p-6 sm:p-8 mb-8">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 bg-gradient-to-br from-accent-cyan/20 to-primary-500/20 rounded-xl flex items-center justify-center">
            ...
            <div className="space-y-4 sm:space-y-5">
              {Object.entries(resultData.ats_score.breakdown).map(([key, item]) => {
                ...
                const barColor = pct >= 80 ? 'bg-gradient-to-r from-emerald-400 to-green-500' : pct >= 50 ? 'bg-gradient-to-r from-amber-400 to-orange-500' : 'bg-gradient-to-r from-red-400 to-red-500';
                return (
                  <div key={key}>
                    <div className="flex flex-col sm:flex-row sm:justify-between text-sm mb-1.5 gap-0.5 sm:gap-0">
                      <span className="font-semibold text-gray-700 capitalize">{key.replace(/_/g, ' ')}</span>
                      <span className="text-gray-500 text-xs sm:text-sm">{item.score}/{item.max} &middot; {item.detail}</span>
                    </div>
                    <div className="w-full bg-gray-100 rounded-full h-3 sm:h-4 overflow-hidden">
                      <div className={`h-full rounded-full transition-all duration-700 ${barColor} shadow-sm`} style={{ width: `${pct}%` }} />
                    </div>
                  </div>
                );
              })}
              <div className="pt-5 sm:pt-6 border-t border-gray-100">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`w-3 h-3 rounded-full ${resultData.ats_score.score >= 70 ? 'bg-emerald-400 shadow-sm shadow-emerald-400/50' : resultData.ats_score.score >= 50 ? 'bg-amber-400 shadow-sm shadow-amber-400/50' : 'bg-red-400 shadow-sm shadow-red-400/50'}`} />
                    <span className={`text-2xl font-extrabold ${resultData.ats_score.score >= 70 ? 'text-emerald-600' : resultData.ats_score.score >= 50 ? 'text-amber-600' : 'text-red-600'}`}>
            ...
            {resultData.ats_score.suggestions?.length > 0 && (
              <div className="mt-6 p-5 sm:p-6 bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl border border-amber-200/60">
                ...
                <ul className="space-y-2.5">
                  {resultData.ats_score.suggestions.map((s, i) => (
                    <li key={i} className="text-sm text-amber-800 flex items-start gap-2">
                      <span className="mt-1.5 w-1.5 h-1.5 bg-amber-400 rounded-full flex-shrink-0 shadow-sm shadow-amber-400/50" />
                      {s}
                    </li>
                  ))}
                </ul>
