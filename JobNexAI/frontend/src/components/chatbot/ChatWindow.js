...
const ChatWindow = ({ onClose }) => {
  ...
  return (
    <div ref={chatRef} className={`${sizeClass} glass-strong backdrop-blur-2xl rounded-[2rem] mx-2 sm:mx-4 shadow-glass-xl flex flex-col overflow-hidden animate-slide-up`}>
      {/* Header */}
      <div className="flex-shrink-0 bg-gradient-to-r from-accent-cyan via-primary-500 to-accent-violet rounded-t-2xl px-5 py-4 flex items-center justify-between">
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto px-4 py-4 bg-gray-50/30 scrollbar-hide">
        {messages.map((msg, index) => {
          if (msg.role === 'system') {
            return (
              <div key={index} className="flex justify-center mb-3">
                <div className="glass rounded-xl px-4 py-2 text-xs text-primary-700 max-w-[90%] text-center">
                  {msg.content}
                </div>
              </div>
            );
      ...
      {showSuggestions && historyLoaded && !isLoading && messages.filter(m => m.role !== 'system').length <= 1 && (
        <div className="flex-shrink-0 border-t border-white/20 bg-white/50 backdrop-blur-xl">
          <SuggestedPrompts role={userRole} onSelect={handlePromptSelect} />
        </div>
      )}
      {/* Input */}
      <div className="flex-shrink-0 border-t border-white/20 glass px-4 py-3">
        <div className="flex items-center gap-2">
          <input ref={inputRef} type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown}
            placeholder="Type your message..." disabled={isLoading}
            className="flex-1 px-4 py-2.5 input-field disabled:opacity-50"
          />
