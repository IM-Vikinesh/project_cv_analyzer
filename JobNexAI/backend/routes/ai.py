...
def analyze():
    try:
        ...
        increment_daily_usage(user_id, usage_type='ai_other')
...
def ai_interview():
    ...
    try:
        ...
        if user_id and action != 'evaluate':
            limit_err = check_ai_limit(user_id, usage_type='interview', daily_limit=1)
            if limit_err:
                return jsonify({'error': limit_err, 'limit_exceeded': True}), 429
...
Example: ["Question 1?", "Question 2?", "Question 3?", "Question 4?", "Question 5?"]"""
            ...
            if user_id:
                increment_daily_usage(user_id, usage_type='interview')
